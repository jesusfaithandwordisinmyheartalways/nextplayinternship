...
- Name: homepage loads with the main heading
- Location: /Users/andrewjohnson/InternshipNextPlayNation/client/tests/example.spec.ts:7:5
...
Locator: locator('h1')
Expected pattern: /next play nation/i
Received: <element(s) not found>
Call log:
  - expect.toHaveText with timeout 5000ms
  - waiting for locator('h1')

    at /Users/andrewjohnson/InternshipNextPlayNation/client/tests/example.spec.ts:10:25
```
...
```ts
   ...
   6 |
   7 | test('homepage loads with the main heading', async ({ page }) => {
   8 |   await page.goto('https://nextplayinternshipclient.onrender.com/'); // Make sure your app is running here
   9 |   const heading = await page.locator('h1');
> 10 |   await expect(heading).toHaveText(/next play nation/i); // Adjust the text to match exactly
     |                         ^ Error: Timed out 5000ms waiting for expect(locator).toHaveText(expected)
  11 | });
```