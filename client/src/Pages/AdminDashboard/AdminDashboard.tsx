...
const AdminDashboard: React.FC = () => {
  ...
  const fetchAll = async () => {
    const res = await fetch('http://localhost:3001/calendar/calendar-events');
  ...
  useEffect(() => {
    const checkAuth = async () => {
      try {
        const res = await fetch('http://localhost:3001/auth/admin-authentication', {
          credentials: 'include',
        });
        const data = await res.json();
  ...
  useEffect(() => {
    ...
    const AdminDashboardClientError = async (errorType:string, errorDetail: any) => {
      try {
        await fetch('http://localhost:3001/admin-dashboard-client-error', {
          method: 'POST',
  ...
  const handleAdd = async () => {
    ...
    try {
      await fetch('http://localhost:3001/calendar/create-event', {
        method: 'POST',
        headers: {
  ...
  const handleUpdate = async () => {
    ...
    try {
      await fetch(`http://localhost:3001/calendar/update-event/${editingId}`, {
        method: 'PUT',
        headers: {
          'Content-Type': 'application/json',
          Authorization: `Bearer ${token}`,
  ...
  const handleDelete = async (id: string) => {
    await fetch(`http://localhost:3001/calendar/delete-event/${id}`, {
      method: 'DELETE',
      headers: {
        Authorization: `Bearer ${token}`,
      },
  ...
  const adminLogout = async (e: React.FormEvent) => {
    e.preventDefault();
    try {
      const response = await fetch('http://localhost:3001/logout/admin-logout', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        credentials: 'include',
      });
